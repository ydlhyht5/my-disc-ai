...
import { AnalysisReport as ReportType, Language } from '../types.ts';
...
export const AnalysisReport: React.FC<Props> = ({ report, onReset, lang }) => {
  const t = {
    zh: {
      basedOn: `基于 ${report.testType} 的深度分析`,
      youAre: "您是一个",
      traits: "核心行为特质",
      strengths: "天赋与独特优势",
      growth: "盲点与提升方向",
      career: "职场/长期成长建议",
      interpersonal: "人际互动深度指南",
      reset: "分析下一份测试报告",
      disclaimer: "此报告由 AI 基于截图内容进行心理学推理，仅供个人成长参考"
    },
    en: {
      basedOn: `In-depth analysis based on ${report.testType}`,
      youAre: "You are a",
      traits: "Core Traits",
      strengths: "Talents & Strengths",
      growth: "Blind Spots & Growth",
      career: "Career & Growth Advice",
      interpersonal: "Interpersonal Interaction Guide",
      reset: "Analyze Next Report",
      disclaimer: "This report is generated by AI based on psychological reasoning and is for reference only."
    }
  }[lang];
    ...
    <div className="max-w-5xl mx-auto p-6 space-y-12 pb-24">
      <header className="text-center space-y-6">
        ...
        <h1 className="text-4xl md:text-6xl font-black text-gray-900 leading-tight tracking-tight">
          {t.youAre} <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-600 via-purple-600 to-pink-600">{report.overallTitle}</span>
        </h1>
        <div className="relative max-w-3xl mx-auto">
          <div className="absolute -left-4 top-0 text-6xl text-indigo-100 font-serif opacity-50">“</div>
          <p className="text-lg text-gray-600 leading-relaxed px-6 whitespace-pre-wrap text-justify italic">
            {report.summary}
          </p>
          <div className="absolute -right-4 bottom-0 text-6xl text-indigo-100 font-serif opacity-50">”</div>
        </div>
      </header>
            ...
            {report.traits.map((trait, idx) => (
              <li key={idx} className="group p-4 bg-slate-50 rounded-2xl border border-slate-100 hover:bg-white hover:shadow-md transition-all">
                <p className="text-gray-700 leading-relaxed font-medium flex items-center">
                  <span className="shrink-0 w-1.5 h-1.5 rounded-full bg-indigo-500 mr-3"></span>
                  {trait}
                </p>
              </li>
            ))}
          ...
          <div className="space-y-4">
            {report.growthAreas.map((g, idx) => (
              <div key={idx} className="flex items-start p-4 bg-orange-50/30 rounded-2xl border border-orange-100">
                <span className="text-orange-500 mr-3 text-lg shrink-0">💡</span>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-gray-900 via-slate-900 to-indigo-900 p-10 md:p-14 rounded-[3rem] shadow-2xl text-white">
        <div className="relative z-10">
          <div className="flex items-center gap-4 mb-8">
            ...
            <h2 className="text-3xl font-black tracking-tight">{t.interpersonal}</h2>
          </div>
      ...
      <div className="flex flex-col items-center gap-6">
        ...
        >
          <span className="relative z-10">{t.reset}</span>
        ...
        <p className="text-gray-400 text-sm text-center px-6">{t.disclaimer}</p>
      </div>
    </div>
  );
};
